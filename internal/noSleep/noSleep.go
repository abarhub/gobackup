...
func PasSleep() {
	...
	log.Printf("Starting keep alive poll... (silence)")
	for {
		select {
		case <-arret:
			log.Printf("arret du no sleep ...")
			changementEtatSleep(setThreadExecStateProc, false)
			log.Printf("Fin du no sleep")
			return
		case <-pulse.C:
...
func changementEtatSleep(setThreadExecStateProc *syscall.LazyProc, veille bool) {
	...
	if err != nil {
		...
		if s != "L’opération a réussi." && s != "The operation completed successfully." {
			log.Printf("Erreur pour changer l'état de veille: %v", err)
		}
	}
}
